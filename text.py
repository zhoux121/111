...
#sift = cv.xfeatures2d.SIFT_create(contrastThreshold=0.03)
...
class SIFTDataset(Dataset):
    ...
    def load_sift(self, img):
        ...
        for i in range(1000):
            ...
            fea = list(des[i])
            if self.isEmpty(feature[a][b][:32]):
                feature[a][b][:32] = fea
                used.append(i)
    ...
    def test(self, pretrained=False):
        ...
        for cnt, items in enumerate(self.test_loader):
            ...
            if cnt < 100:
                ...
                cv.imwrite('res/' + self.choice + '_results/Io_%06d.jpg' % (cnt+1), Io[0])
        if pretrained:
